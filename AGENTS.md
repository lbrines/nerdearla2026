...
- Phase 3 Review 2 is complete: `start`, `healthy-check`, `fault-on`, `verify`, `fault-off`, and `stop` are available. Phase 3.3 adds `reset`; it recreates the fixed lab project with owned volumes removed, then starts and verifies healthy. `record-ready` remains an explicit failing placeholder pending later integration work. Phase 3 is not complete.
- Phase 4.1 implements checkout, pricing, and gateway metrics, validated directly at `/metrics` by `./tests/observability.sh`. Phase 4.2 implements checkout and pricing JSONL logs; Phase 4.3A adds gateway and load-generator JSONL logs and their shared-volume mounts. Phase 4.3B verifies cross-service runtime correlation and the shared-log lifecycle: `stop` retains logs and `reset` recreates them. Phase 5.1 adds Prometheus with an explicit 5-second scrape interval and verifies healthy and degraded PromQL observations through `./tests/prometheus.sh`. Phase 5.2 adds automatically provisioned Grafana at `http://127.0.0.1:3000`, verified by `./tests/grafana.sh`. Phase 6.1 adds the opt-in, one-shot investigator sandbox with a read-only log mount and validates its technical boundary through `./tests/isolation.sh`. Phase 7A completes the Baseline, Protocol, and Internal Operator contract. Phase 7B implements locally testable one-shot Baseline and Protocol profiles, common neutral assets, persistent Protocol investigation evidence, and a separate Internal Operator runbook; `./tests/profiles.sh` and `./tests/isolation.sh` validate the local profiles. Phase 7B remains incomplete because unrestricted Internet access can reach the currently public repository containing restricted implementation details. Repository privacy or public-source handling will be decided after manual testing; do not begin Phase 8 before that decision and the section 22 boundary pass. The integrated Phase 4 acceptance remains incomplete, so Phase 4 is not complete.
...
11. Keep the Internal Operator runbook under `operator/`; never copy or mount it into an investigator profile.
...
- `versions.env` is the single source of image and tool version pins; Go module dependencies are pinned in `go.mod` with generated checksums in `go.sum`.
- Never use `latest`.
